  ...
  return (
    <div ref={mainRef} className={`${styles.main} ${styles[layoutMode]}`}>
        <div className={styles.timeDiv}>
            ...
            <p className={styles.doubleDot}>:</p>
            <p>{String(Math.floor((time % 3600) % 60)).slice(0, 2).padStart(2, "0")}</p>
        </div>
        <div className={styles.buttonDiv}>
            {
              (layoutMode === "small" || !settingButton) && (<button onClick={handleReset} className={styles.button}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d={resetPath}/></svg></button>)
            }
            {
              layoutMode !== "small" && settingButton && (<button onClick={changeResetButtonState} className={styles.button}><span>Break</span><span>{resetButtonState}</span></button>)
            }
            {
              (layoutMode === "small" || !settingButton) && (<button onClick={(e) => { pausePlay() }} className={`${styles.button} play-button`}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path className="play-path" d={playPath}/></svg></button>)
            }
            {
              layoutMode !== "small" && settingButton && (<button onClick={changePlayButtonState} className={`${styles.button} play-button`}><span>Work</span><span>{playButtonState}</span></button>)
            }
            ...
            <button onClick={toggleSetting} className={`${styles.button} ${styles.settings}`}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d={settingPath}/></svg></button>
        </div>    
    </div>
  )
}
