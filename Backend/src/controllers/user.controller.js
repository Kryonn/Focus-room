    ...
    async loginUser(req, res) {
        ...
        try {
            ...
            if(!selectResponse.data.rowCount) {
                return res.status(404).json({ error: true, msg: "Usuário inexistente" });
            }

            const user = selectResponse.data.rows[0];

            if(user.password !== password) {
                return res.status(401).json({ error: true, msg: "Usuário ou senha inválidos" });
            }

            res.status(200).json({ error: false, msg: "Usuário autenticado com sucesso" });
        } catch(error) {
